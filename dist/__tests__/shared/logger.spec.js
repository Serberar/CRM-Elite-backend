...
describe('Logger', () => {
    ...
    describe('Transport Configuration', () => {
        ...
        it('should have console transport', () => {
            const consoleTransport = logger.transports.find((t) => t instanceof winston_1.default.transports.Console);
            expect(consoleTransport).toBeDefined();
        });
        it('should have file transports', () => {
            const fileTransports = logger.transports.filter((t) => t instanceof winston_1.default.transports.File);
            expect(fileTransports.length).toBeGreaterThanOrEqual(2); // combined.log y error.log
        });
        it('should configure combined log file', () => {
            const combinedTransport = logger.transports.find((t) => t instanceof winston_1.default.transports.File && t.filename && t.filename.includes('combined.log'));
        ...
        it('should configure error log file', () => {
            const errorTransport = logger.transports.find((t) => t instanceof winston_1.default.transports.File && t.filename && t.filename.includes('error.log'));
        ...
        it('should use paths for log files', () => {
            ...
            fileTransports.forEach((transport) => {
                if (transport.filename) {
                    ...
                    const isValidLogFile = transport.filename.includes('combined.log') || transport.filename.includes('error.log');
    ...
    describe('Winston Configuration Verification', () => {
        ...
        it('should have winston transport instances', () => {
            const logger = require('../../infrastructure/observability/logger/logger').default;
            const hasConsoleTransport = logger.transports.some((t) => t instanceof winston_1.default.transports.Console);
            const hasFileTransport = logger.transports.some((t) => t instanceof winston_1.default.transports.File);
